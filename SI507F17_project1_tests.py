...
class TestCardClass(unittest.TestCase):
    ...
    def test_Card_variables(self):
        self.assertEqual(
            self.card1.suit_names, [
                'Diamonds', 'Clubs',
                'Hearts', 'Spades'], "test Card's suit_names content")

        self.assertEqual(self.card1.rank_levels, list(range(1, 14)),
                         "test Card's rank_levels content")
        ...
        self.assertTrue(
            isinstance(
                self.card1.faces,
                dict),
            "test Card's faces type")
        shared_items = set(
            self.card1.faces.items()) & set(
            expected_dict.items())
    ...
    def test_Card_constructor(self):
        ...
        self.assertEqual(
            self.card1.suit,
            'Diamonds',
            "test Card's default constructor")

        self.assertEqual(self.card1.rank, 2,
                         "test Card's default constructor")
        ...
        self.assertEqual(
            self.card2.suit,
            'Spades',
            "test Card's constructor with arguments")
    ...
    def test_Card_str_method(self):
        self.assertEqual(str(self.card1),
                         "2 of Diamonds",
                         "test whether Card string method " +
...
class TestDeckClass(unittest.TestCase):
    ...
    def test_Deck_constructor(self):
        ...
        for i in range(1, 14):
            for j in range(4):
                self.assertTrue(str(Card(j, i)) in str(
                    self.deck), "test cards' content")
    ...
    def test_Deck_string_method(self):
        ...
        self.assertIsInstance(
            deck_name,
            str,
            "test __str__ method's returned type,\
            actually a little redundant")
        lines = deck_name.split('\n')
        self.assertEqual(
            len(lines),
            52,
            "test whether __str__ method returns 52 lines")
        for i in range(1, 14):
            for j in range(4):
                self.assertTrue(str(Card(j, i)) in lines,
                                "test __str__ method's returned content")
    ...
    def test_Dectk_pop_card_method(self):
        ...
        self.assertEqual(
            str(card1),
            str(expected_card1),
            "test the defualt behavior of pop method")
        for i in range(1, 14):
            for j in range(4):
                ...
                if str(Card(j, i)) == str(card1):
                    continue
                self.assertTrue(str(Card(j, i)) in str(
                    self.deck), "test defualt behavior of pop method")
        ...
        self.assertEqual(
            str(card2),
            str(expected_card2),
            "test pop method by passing user-defined position")
        ...
        for i in range(50):
            self.assertTrue(
                self.deck.pop_card(),
                "test whether Deck object can keep popping when it has cards")
    ...
    def test_Deck_shuffle_method(self):
        ...
        self.assertIsInstance(
            self.deck.cards,
            list,
            "test shuffuled cards' type")
        self.assertEqual(len(self.deck.cards), 52,
                         "test shuffuled cards' length")
        for i in range(1, 14):
            for j in range(4):
                self.assertTrue(str(Card(j, i)) in str(
                    self.deck), "test shuffuled cards' content")
    ...
    def test_Deck_replace_card_method(self):
        ...
        self.assertEqual(str(self.deck), expected_deck_name,
                         "test whether 'adding new card' functions well")
        deck_name = str(self.deck)

        self.deck.replace_card(card30)
        self.assertEqual(deck_name, str(self.deck),
                         "test whether 'adding old card' functions well")
    ...
    def test_Deck_sort_cards_method(self):
        ...
        expected_string = "{}\n{}\n{}\n{}".format(
            str(Card(0, 12)), str(Card(1, 8)),
            str(Card(2, 2)), str(Card(3, 4)))
        self.assertEqual(str(self.deck), expected_string,
                         "test sort_card method")
    ...
    def test_Deck_deal_hand_method(self):
        ...
        self.assertIsInstance(
            hand, list, "test the type of the returned value")
        self.assertEqual(len(hand), 5, "test the length of the returned list")
        self.assertIsInstance(
            hand[0],
            Card,
            "test the type of elements in the returned list")
        indices = []
        for i in range(5):
            ...
            self.assertFalse(index in indices,
                             "test if 'hand' has any duplicate cards")
...
class TestPlayWarGameFunction(unittest.TestCase):
    def test_play_war_game_function(self):
        ...
        self.assertIsInstance(
            result, tuple, "test first returned value's type")
...
class TestShowSong(unittest.TestCase):
    ...
    def test_show_song_function(self):
        self.assertIsInstance(
            self.song1,
            helper_functions.Song,
            "test show_song function(defualt)")
        self.assertIsInstance(
            self.song2,
            helper_functions.Song,
            "test_show_song function")
